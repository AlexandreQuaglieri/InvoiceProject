...
export async function POST(request: NextRequest) {
  try {
    ...
    // Valider chaque redirect_uri
    for (const uri of redirect_uris) {
      if (!isValidRedirectUri(uri) && !isClaudeCallback(uri)) {
        return NextResponse.json(
          {
            error: 'invalid_redirect_uri',
            error_description: `Invalid redirect_uri: ${uri}. Must be localhost or HTTPS.`,
          },
          { status: 400 }
        )
      }
    ...
    // Inclure le secret seulement si généré
    if (clientSecret) {
